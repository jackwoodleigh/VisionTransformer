...
class WindowedContextCA(WindowedAttention):
    def __init__(self, window_size, dim, num_heads=4, attn_drop=0.0, proj_drop=0.):
        ...
        self.kv_proj = LastChannelConv2D(dim, 2*dim, self.summary_size, self.summary_size, groups=num_heads)
        self.q_proj = LastChannelConv2D(dim, dim, self.summary_size, self.summary_size, groups=num_heads)

        self.unfold = nn.Unfold(kernel_size=(self.kernal_size * self.win_s, self.kernal_size * self.win_s), stride=self.win_s, padding=self.win_s)

        self.pe = RelativePositionalEncoding(q_size=self.win_s, k_size=self.kernal_size * self.win_s, num_heads=num_heads)
    ...
    def overlapping_windows(self, x):
        ...
        # B, C * K^2 * win_s^2, H/win * W/win -> B, C, K^2 * win_s^2, H/win * W/win
        x = x.view(B, C, self.kernal_size**2 * self.win_s**2, -1)

        # B, C, K^2 * win_s^2, H/win * W/win -> B, H/win * W/win, K^2 * win_s^2, C
    ...
    def forward(self, x_wsa, x_original):
        ...
        # B, H, W, C -> B, H/S, W/S, C -> B, H/win, W/win, (win/S)^2, C -> B * H/win * W/win, (win/S)^2, C
        q = self.window_partition(self.q_proj(x_wsa), self.win_s, keep_batch=True).view(B * (H//self.win) * (W//self.win), self.win_s**2, C)

        # B, H, W, C -> B, H/S, W/S, C -> B * (H/win) * (W/win), K^2 * (win/S)^2, C
        kv = self.kv_proj(x_original)
        k, v = self.overlapping_windows(kv).view(B * (H//self.win) * (W//self.win), self.kernal_size**2 * self.win_s**2, 2*C).chunk(2, dim=-1)

        # B * (H/win) * (W/win), K^2 * (win/S)^2, C -> B * (H/win) * (W/win), n_heads, K^2 * (win/S)^2, C/n_heads
        q, k, v = self.headify(q, k, v)

        x_cross = torch.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=self.pe())

        # B * (H/win) * (W/win), n_heads, K^2 * (win/S)^2, C/n_heads -> B, (H/win) * (W/win), (win/S)^2, C
        x_cross = self.deheadify(x_cross).view(B, (H//self.win) * (W//self.win), self.win_s**2, C)

        # B, (H/win) * (W/win), (win/S)^2, C -> B, H/S, W/S, C -> B, H/S, 1, W/S, 1, C
        x_cross = self.reverse_window_partition(x_cross, self.win_s, H_S, W_S, with_batch=True).view(B, H_S, 1, W_S, 1, C)
