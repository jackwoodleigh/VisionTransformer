        ...
        for e in range(epochs):
            ...
            for i, (hr, lr) in enumerate(pbar):
                ...
                if (i + 1) % accumulation_steps == 0 or (i + 1) == n:
                    ...
                    self.scaler.update()
                    self.optimizer.zero_grad(set_to_none=True)
                    self.ema.step_ema(ema_model=self.ema_model, model=self.model, start_step=len(train_loader)*ema_start_epoch // accumulation_steps)
            ...
            with torch.no_grad():
                ...
                for i, (hr, lr) in enumerate(pbar):
                    loss, hr_p = self.predict(hr, lr, pl_scale, fft_loss_scale, use_ema_model=e+1 > ema_start_epoch)
